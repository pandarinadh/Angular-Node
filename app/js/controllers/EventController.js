...
app.controller('EventController', function EventController($scope)
{
    ...
    y = d3.scale.linear().range([$scope.graph.height - $scope.graph.MARGINS.top, $scope.graph.MARGINS.bottom]);
...
vis.append('svg:g')
  ...
  .call(yAxis);
/*
  for(let l in $scope.chartPoints)
  {
    vis.append('svg:path')
    .attr('d', $scope.lineFunc(l.points))
    .attr('stroke', l.color)
    .attr('stroke-width', 2)
    .attr('fill', 'none');
  }
